...
export default function DemographicChart() {
    ...
    return (
        <div style={{ position: 'relative', width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', justifyContent: 'center', padding: '4vh 4vw', fontFamily: FONT, backgroundColor: 'transparent' }}>
            ...
            {/* Content Container */}
            <div style={{ width: '100%', maxWidth: '1200px', display: 'flex', flexDirection: 'column', gap: '3rem', zIndex: 1 }}>

            {/* Decline overview */}
            <div style={{ marginBottom: '2.5rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '0.75rem' }}>
                    <span style={{ fontSize: '0.75rem', color: '#999', fontWeight: 400, width: '80px', fontFamily: FONT }}>157 AD</span>
                    <div style={{ flex: 1, position: 'relative', height: '28px' }}>
                        ...
                        />
                        <span style={{ position: 'absolute', right: '8px', top: '50%', transform: 'translateY(-50%)', fontSize: '0.75rem', color: '#666', fontWeight: 400, fontFamily: FONT }}>
                            10,670,000 households (Late Han)
                        </span>
                    </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    <span style={{ fontSize: '0.75rem', color: '#999', fontWeight: 400, width: '80px', fontFamily: FONT }}>280 AD</span>
                    <div style={{ flex: 1, position: 'relative', height: '28px' }}>
                        <motion.div
                            initial={{ width: 0 }}
                        ...
                        />
                        <span style={{ position: 'absolute', left: `${(TOTAL_JIN / TOTAL_HAN) * 100 + 1}%`, top: '50%', transform: 'translateY(-50%)', fontSize: '0.75rem', color: '#666', fontWeight: 400, fontFamily: FONT, whiteSpace: 'nowrap' }}>
                            2,450,000 households (Jin) — 77% decline
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: '2rem' }}>
                {KINGDOMS.map((k, i) => {
                    ...
                    // Wei bar is ~100%, so label goes inside; Shu/Wu are short, label outside
                    ...
                    return (
                        ...
                        >
                            <div style={{ width: '80px', textAlign: 'left' }}>
                                <span style={{ fontSize: '1rem', fontWeight: 400, color: isHovered ? k.color : '#333', transition: 'color 0.3s', fontFamily: FONT }}>
                                    {k.label}
                                </span>
                                <br />
                                <span style={{ fontSize: '0.7rem', color: '#aaa', fontFamily: FONT, fontWeight: 400 }}>{k.year}</span>
                            </div>
                            <div style={{ flex: 1, position: 'relative', height: '40px' }}>
                                ...
                                >
                                    {labelInside && (
                                        ...
                                        >
                                            {k.households.toLocaleString()} households · troops {k.troops}
                                        </motion.span>
                                    )}
                                </motion.div>
                                {!labelInside && (
