...
// Remove subSlide state
code = code.replace(/const \[subSlide, setSubSlide\] = useState\(0\);\n/, '');
code = code.replace(/const totalSlides = 18;/, 'const totalSlides = 21;');

// Simplify wheel logic
code = code.replace(/if \(e.deltaY > 30\) \{[\s\S]*?\} else if \(e\.deltaY < -30\)/, `if (e.deltaY > 30) {
                setCurrentSlide(prev => Math.min(prev + 1, totalSlides - 1));
            } else if (e.deltaY < -30)`);
code = code.replace(/if \(e.deltaY < -30\) \{[\s\S]*?\} else \{/, `if (e.deltaY < -30) {
                setCurrentSlide(prev => Math.max(prev - 1, 0));
                if (currentSlide === 0) onClose();
            }`);

// Simplify key logic
code = code.replace(/if \(e\.key === "ArrowDown" \|\| e\.key === "ArrowRight"\) \{[\s\S]*?\} else if \(e\.key === "ArrowUp"/, `if (e.key === "ArrowDown" || e.key === "ArrowRight") {
                isScrolling.current = true;
                setCurrentSlide(prev => Math.min(prev + 1, totalSlides - 1));
                setTimeout(() => { isScrolling.current = false; }, 800);
            } else if (e.key === "ArrowUp"`);
code = code.replace(/else if \(e\.key === "ArrowUp" \|\| e\.key === "ArrowLeft"\) \{[\s\S]*?\} else if \(e\.key === "Escape"\)/, `else if (e.key === "ArrowUp" || e.key === "ArrowLeft") {
                isScrolling.current = true;
                setCurrentSlide(prev => Math.max(prev - 1, 0));
                setTimeout(() => { isScrolling.current = false; }, 800);
            } else if (e.key === "Escape")`);

// Simplify touch logic
code = code.replace(/if \(diff > 0\) \{[\s\S]*?\} else \{[\s\S]*?\}/, `if (diff > 0) {
                    setCurrentSlide(prev => Math.min(prev + 1, totalSlides - 1));
                } else {
                    setCurrentSlide(prev => Math.max(prev - 1, 0));
                }`);

// Fix indicator onClick
code = code.replace(/onClick=\{.*?setCurrentSlide\(idx\); setSubSlide\(0\);.*?\}/, `onClick={() => setCurrentSlide(idx)}`);
...
// Map components directly based on absolute currentSlide
// Originally 11 to 16 -> now 15 to 20
code = code.replace(/currentSlide === 11 && <DynamicTimelineChart/g, 'currentSlide === 15 && <DynamicTimelineChart');
code = code.replace(/currentSlide === 12 && <DemographicChart/g, 'currentSlide === 16 && <DemographicChart');
code = code.replace(/currentSlide === 13 && <WarlordFlowChart/g, 'currentSlide === 17 && <WarlordFlowChart');
code = code.replace(/currentSlide === 14 && <BattleBubbleChart/g, 'currentSlide === 18 && <BattleBubbleChart');
code = code.replace(/currentSlide === 15 && <RelationshipNetwork/g, 'currentSlide === 19 && <RelationshipNetwork');
code = code.replace(/currentSlide === 16 && <TerritorialMap/g, 'currentSlide === 20 && <TerritorialMap');
